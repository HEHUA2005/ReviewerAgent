...
"""
    ...
    def _extract_review_from_text(self, paper_info: Dict, text: str) -> str:
        ...
        weaknesses = self._extract_list_section(text, ["weaknesses", "缺点", "weakness"])
    ...
    def _extract_section(self, text: str, keywords: List[str], default: str) -> str:
        ...
        for keyword in keywords:
            ...
            if start_idx != -1:
                ...
                # 查找下一个部分的开始
                next_sections = ["strengths", "weaknesses", "questions", "优点", "缺点", "问题"]
                end_idx = len(text)
    ...
    def _extract_list_section(self, text: str, keywords: List[str]) -> List[str]:
        ...
        for keyword in keywords:
            ...
            if start_idx != -1:
                ...
                start_idx += len(keyword)
                # 查找下一个部分的开始
                next_sections = ["summary", "strengths", "weaknesses", "questions", "摘要", "优点", "缺点", "问题"]
                end_idx = len(text)
                
                for next_section in next_sections:
                    next_idx = text_lower.find(next_section.lower(), start_idx)
                    if next_idx != -1 and next_idx < end_idx and next_section.lower() != keyword.lower():
                        end_idx = next_idx
