...
from a2a.types import (
    TaskStatusUpdateEvent, TaskStatus, TaskState, Task,
    SendMessageRequest, SendMessageSuccessResponse, JSONRPCErrorResponse,
    TextPart, Part, Message, Role, MessageSendParams, MessageSendConfiguration,
    GetTaskRequest, TaskQueryParams, GetTaskSuccessResponse, FilePart
)
...
class ReviewerAgent:
    ...
    def __init__(self):
        ...
        self.llm_reviewer = LLMReviewer(llm_client=self.llm_client)
...
"""
    ...
    def _generate_cache_filename(self, paper_result: 'PaperSearchResult') -> str:
        ...
        authors = "_".join(paper_result.authors[:2]) if paper_result.authors else "unknown_author"
        ...
        content_hash = hashlib.md5(f"{paper_result.url}_{paper_result.pdf_url}".encode()).hexdigest()[:8]
    ...
    async def _cache_pdf(self, pdf_data: bytes, paper_result: 'PaperSearchResult') -> Path:
        """
    ...
    async def _load_cached_pdf(self, paper_result: 'PaperSearchResult') -> Optional[bytes]:
        """
    ...
    async def search_papers(self, query: str, max_results: int = 5) -> List[PaperSearchResult]:
        """Search for papers."""
    ...
    async def handle_pdf_review(self, pdf_data: bytes, task_id: str) -> str:
        ...
        formatted_review = await self.llm_reviewer.generate_review(paper_info, paper_text)
    ...
    async def handle_paper_selection(self, selection: str, context_id: str) -> str:
        ...
        if context_id not in self.tasks or self.tasks[context_id]["state"] != "search_results":
        ...
        if selected_paper.get("pdf_url"):
            ...
            if pdf_data is None:
                ...
                logger.info(f"PDF not in cache, downloading from: {selected_paper['pdf_url']}")
                ...
                if pdf_data:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to cache PDF, continuing with review: {e}")
                        pdf_available = True
                else:
                    logger.warning(f"Could not download PDF from: {selected_paper['pdf_url']}")
                    pdf_available = False
            else:
        ...
        if not pdf_available or not paper_text:
            logger.info("No PDF available, generating review based on abstract and metadata")
        ...
        try:
            ...
            review_result = await self.review_engine.analyze_paper(paper_text, selected_paper)
            ...
            try:
                formatted_review = await self.llm_reviewer.generate_review(selected_paper, paper_text)
...
"""
        ...
        for criterion, score in scores.items():
            if criterion != "overall":
                formatted_review += f"- **{criterion.replace('_', ' ').title()}**: {score}/10\n"
...
"""
    ...
    async def handle_review_template(self) -> str:
        ...
        formatted_template = OutputFormatter.format_review_criteria(REVIEW_CRITERIA)
        ...
        try:
            ...
            # Handle text-based requests
                ...
                if task_type == "welcome":
                    ...
                    response = await self.agent.handle_paper_selection(selection, context_id)
                    ...
                    if url:
                        ...
                        pdf_data = await self.agent.paper_search.download_paper_from_url(url)
                        if pdf_data:
                            response = await self.agent.handle_pdf_review(pdf_data, context_id)
                        else:
